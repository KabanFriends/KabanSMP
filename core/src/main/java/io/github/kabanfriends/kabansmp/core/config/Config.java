...
public abstract class Config {
    ...
    public void load() {
        ...
        if (!configFile.exists()) {
            KabanSMP.getInstance().getLogger().log(Level.WARNING, "Config " + configFile.getName() + " could not be found!");
            return;
        }

        KabanSMP.getInstance().getLogger().log(Level.INFO, "Loading config: " + configFile.getName());
        ...
        try {
            ...
            for (ConfigField field : fields) {
                ...
                try {
                    ...
                    KabanSMP.getInstance().getLogger().log(Level.WARNING, "Failed to parse config entry: " + field.id + " (" + configFile.getName() + ")");
                }
            }
        } catch (FileNotFoundException e) {
            KabanSMP.getInstance().getLogger().log(Level.WARNING, "Failed to read config " + configFile.getName());
            e.printStackTrace();
        } catch (JsonParseException e) {
            KabanSMP.getInstance().getLogger().log(Level.WARNING, "Invalid config json: " + configFile.getName());
            e.printStackTrace();
        }
    }
}
